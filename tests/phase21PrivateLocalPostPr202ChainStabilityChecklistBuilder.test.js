...
const engine = require("../phase21-9-private-local-post-pr202-chain-stability-checklist-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-9-private-local-post-pr202-chain-stability-checklist-db.json");
const summaryDb = readJson("phase21-9-private-local-post-pr202-chain-stability-checklist-summary-db.json");
...
assert.equal(engine.PHASE, "Phase21-9");
assert.equal(engine.CHECKLIST_NAME, "Private Local Post PR202 Chain Stability Checklist");
assert.equal(engine.EXECUTION_POLICY, "PLAN_ONLY");
assert.equal(engine.PROTECTION_POLICY, "Protected");
assert.equal(engine.PANEL_STATUS, "phase21_9_private_local_post_pr202_chain_stability_checklist_plan_only");
assert.equal(engine.POST_PR202_CHAIN_STATUS, "POST_PR202_CHAIN_STABILITY_READY");
...
assert.equal(engine.PHASE21_8_PREREQUISITE, "POST_PR201_CHAIN_STABILITY_READY");
...
assert.equal(engine.CHECKS.length, 30);

const panel = engine.buildPrivateLocalPostPr202ChainStabilityChecklist({}, () => new Date("2026-07-02T00:00:00.000Z"));
assert.equal(panel.phase, "Phase21-9");
assert.equal(panel.records.length, 30);
...
assert.equal(panel.post_pr202_chain_stability_only, true);
...
assert.equal(panel.phase21_8_post_pr201_chain_stability_checklist_preserved, true);
assert.equal(panel.phase21_8_merge_allowed, false);
assert.equal(panel.phase21_9_push_allowed_without_instruction, false);
assert.equal(panel.phase21_9_pr_creation_allowed_without_instruction, false);
...
assert.equal(summary.total_checks, 30);
assert.equal(summary.confirmed_checks, 17);
assert.equal(summary.manual_review_required_checks, 8);
...
assert.equal(summary.phase21_8_pr202_created, true);
assert.equal(summary.phase21_9_push_allowed_without_instruction, false);
assert.equal(summary.phase21_9_pr_creation_allowed_without_instruction, false);
assert.equal(summary.company_pc_main_clean_confirmed, true);
assert.equal(summary.draft_pr_chain_order_confirmed, true);
assert.equal(summary.pr202_base_compare_chain_required, true);
assert.equal(summary.draft_pr_chain_audit_required, true);
assert.equal(summary.home_pc_post_pr202_confirmation_required, true);
assert.equal(summary.company_pc_post_pr202_confirmation_required, true);
assert.equal(summary.ipad_post_pr202_confirmation_required, true);
...
assert.equal(db.phase, "Phase21-9");
assert.equal(db.postPr202ChainStatus, "POST_PR202_CHAIN_STABILITY_READY");
assert.equal(db.phase21_8_draftPr, "PR #202 Draft");
assert.equal(db.phase21_8_pr202Created, true);
assert.equal(db.phase21_9_pushAllowedWithoutInstruction, false);
assert.equal(db.phase21_9_prCreationAllowedWithoutInstruction, false);
assert.equal(db.companyPcMainCleanConfirmed, true);
assert.equal(db.draftPrChainOrderConfirmed, true);
assert.equal(db.pr202BaseCompareChainRequired, true);
assert.equal(db.draftPrChainAuditRequired, true);
assert.equal(db.homePcPostPr202ConfirmationRequired, true);
assert.equal(db.companyPcPostPr202ConfirmationRequired, true);
assert.equal(db.ipadPostPr202ConfirmationRequired, true);
...
assert.equal(summaryDb.phase21_9_summary.totalChecks, 30);
assert.equal(summaryDb.phase21_9_summary.confirmedChecks, 17);
assert.equal(summaryDb.phase21_9_summary.manualReviewRequiredChecks, 8);
assert.equal(summaryDb.phase21_9_summary.blockedChecks, 5);
assert.equal(summaryDb.phase21_9_summary.phase21_8_postPr201ChainStabilityChecklistPreserved, true);
assert.equal(summaryDb.phase21_9_summary.phase21_8_pr202Created, true);
assert.equal(summaryDb.phase21_9_summary.phase21_9_pushAllowedWithoutInstruction, false);
assert.equal(summaryDb.phase21_9_summary.phase21_9_prCreationAllowedWithoutInstruction, false);
assert.equal(summaryDb.phase21_9_summary.companyPcMainCleanConfirmed, true);
assert.equal(summaryDb.phase21_9_summary.draftPrChainAuditRequired, true);
assert.equal(summaryDb.phase21_9_summary.autoReadyForReviewAllowed, false);
assert.equal(summaryDb.phase21_9_summary.githubPagesSettingChangeAllowed, false);
assert.equal(summaryDb.phase21_9_summary.externalApiConnection, false);
assert.equal(summaryDb.phase21_9_summary.autoExecution, false);
...
assert.equal(phase218Summary.phase21_8_summary.phase21_7_postDraftPrCreationStabilityChecklistPreserved, true);
...
assert.ok(index.includes('id="phase21-9-private-local-post-pr202-chain-stability-checklist-builder"'));
assert.ok(index.includes('<script src="phase21-9-private-local-post-pr202-chain-stability-checklist-builder.js"></script>'));
assert.ok(privateLocal.includes('href="index.html#phase21-9-private-local-post-pr202-chain-stability-checklist-builder"'));
assert.ok(readme.includes("Phase21-9 Private Local Post PR202 Chain Stability Checklist"));
assert.ok(css.includes(".phase21-9-private-local-post-pr202-chain-stability-checklist"));

const builderSource = readText("phase21-9-private-local-post-pr202-chain-stability-checklist-builder.js");
...
for (const forbidden of [
  ...
  assert.equal(builderSource.includes(forbidden), false, `${forbidden} must not appear in Phase21-9 builder`);
}

console.log("phase21-9 private local post PR202 chain stability checklist builder tests passed");
