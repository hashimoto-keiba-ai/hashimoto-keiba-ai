...
assert.strictEqual(OFFICIAL_RELEASE.version, "2.7");
assert.strictEqual(release.version, "2.7");
...
assert.strictEqual(release.releaseScore, 112);
assert.strictEqual(release.releaseStatus, "Official Release v2.7");
assert.strictEqual(storage.readRaw(STORAGE_KEYS.releaseVersion), "2.7");
assert.strictEqual(storage.readRaw(STORAGE_KEYS.releaseStatus), "Official Release v2.7");
assert.strictEqual(storage.readJson(STORAGE_KEYS.officialReleaseReports)[0].version, "2.7");
assert.ok(release.releaseNotes.some((note) => note.includes("Version 2.7")));
