...
const builder = require("../phase21-61-private-local-post-pr255-continuation-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-61-private-local-post-pr255-continuation-check-db.json");
const summary = readJson("phase21-61-private-local-post-pr255-continuation-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-61-private-local-post-pr255-continuation-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-61");
assert.strictEqual(builder.PANEL_STATUS, "phase21_61_private_local_post_pr255_continuation_check_plan_only");
assert.strictEqual(db.phase, "Phase21-61");
assert.strictEqual(summary.status, "PHASE21_61_PRIVATE_LOCAL_POST_PR255_CONTINUATION_CHECK_READY");
assert.strictEqual(db.latestMainHead, "9d419f6");
assert.ok(db.baseline.includes("PR #255 merged"));
assert.ok(db.baseline.includes("home PC main updated / private-local.html confirmed"));
assert.strictEqual(db.previousPhase, "Phase21-60");
assert.strictEqual(db.previousPullRequest, "PR #255");
assert.strictEqual(db.homePcMainUpdatedAndPrivateLocalHtmlConfirmed, true);
assert.strictEqual(db.mainSyncedAfterPr255, true);
assert.strictEqual(db.phase2160MainReflectionConfirmed, true);
...
const panel = builder.buildPrivateLocalPostPr255ContinuationCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, db.baseline);
assert.strictEqual(panel.latestMainHead, "9d419f6");
assert.strictEqual(panel.homePcMainUpdatedAndPrivateLocalHtmlConfirmed, true);
assert.strictEqual(panel.phase2160MainReflectionConfirmed, true);
...
assert.ok(panel.records.some((record) => String(record.label).includes("PR #255 merged")));
assert.ok(panel.records.some((record) => String(record.label).includes("Latest main HEAD is 9d419f6")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostPr255ContinuationCheckPanel"));
assert.ok(index.includes('id="phase21-61-private-local-post-pr255-continuation-check"'));
assert.ok(index.includes('<script src="phase21-61-private-local-post-pr255-continuation-check-builder.js"></script>'));
assert.ok(index.indexOf("phase21-61-private-local-post-pr255-continuation-check") < index.indexOf("phase21-60-private-local-post-pr254-home-pc-continuation-check"));
assert.ok(privateLocal.includes('href="index.html#phase21-61-private-local-post-pr255-continuation-check"'));
assert.ok(privateLocal.indexOf("phase21-61-private-local-post-pr255-continuation-check") < privateLocal.indexOf("phase21-60-private-local-post-pr254-home-pc-continuation-check"));
assert.ok(readme.includes("Phase21-61"));
assert.ok(readme.includes("PR #255 merged"));
assert.ok(readme.includes("home PC main updated / private-local.html confirmed"));
assert.ok(readme.includes("9d419f6"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-61-private-local-post-pr255-continuation-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostPr255ContinuationCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #255 merged")));
assert.ok(rendered.some((line) => line.includes("9d419f6")));
assert.ok(rendered.some((line) => line.includes("No suspicious auto-run script")));

console.log("Phase21-61 private local post PR255 continuation check builder test passed");
