...
const engine = require("../phase21-10-private-local-post-pr203-operation-checklist-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-10-private-local-post-pr203-operation-checklist-db.json");
const summaryDb = readJson("phase21-10-private-local-post-pr203-operation-checklist-summary-db.json");
...
assert.equal(engine.PHASE, "Phase21-10");
assert.equal(engine.CHECKLIST_NAME, "Private Local Post PR203 Operation Checklist");
assert.equal(engine.EXECUTION_POLICY, "PLAN_ONLY");
assert.equal(engine.PROTECTION_POLICY, "Protected");
assert.equal(engine.PANEL_STATUS, "phase21_10_private_local_post_pr203_operation_checklist_plan_only");
assert.equal(engine.POST_PR203_OPERATION_STATUS, "POST_PR203_PRIVATE_LOCAL_OPERATION_READY");
...
assert.equal(engine.CHECKS.length, 32);

const panel = engine.buildPrivateLocalPostPr203OperationChecklist({}, () => new Date("2026-07-02T00:00:00.000Z"));
assert.equal(panel.phase, "Phase21-10");
assert.equal(panel.records.length, 32);
...
assert.equal(panel.post_pr203_operation_checklist_only, true);
...
assert.equal(panel.phase21_9_merge_allowed, false);
assert.equal(panel.phase21_10_push_allowed_without_instruction, false);
assert.equal(panel.phase21_10_pr_creation_allowed_without_instruction, false);
...
assert.equal(summary.total_checks, 32);
assert.equal(summary.confirmed_checks, 18);
assert.equal(summary.manual_review_required_checks, 9);
...
assert.equal(summary.phase21_9_pr203_created, true);
assert.equal(summary.phase21_10_push_allowed_without_instruction, false);
assert.equal(summary.phase21_10_pr_creation_allowed_without_instruction, false);
assert.equal(summary.company_pc_main_clean_confirmed, true);
assert.equal(summary.draft_pr_chain_order_confirmed, true);
assert.equal(summary.pr203_base_compare_chain_required, true);
assert.equal(summary.draft_pr_chain_audit_required, true);
assert.equal(summary.home_pc_post_pr203_confirmation_required, true);
assert.equal(summary.company_pc_post_pr203_confirmation_required, true);
assert.equal(summary.ipad_post_pr203_confirmation_required, true);
...
assert.equal(db.phase, "Phase21-10");
assert.equal(db.postPr203OperationStatus, "POST_PR203_PRIVATE_LOCAL_OPERATION_READY");
assert.equal(db.phase21_9_draftPr, "PR #203 Draft");
assert.equal(db.phase21_9_pr203Created, true);
assert.equal(db.phase21_10_pushAllowedWithoutInstruction, false);
assert.equal(db.phase21_10_prCreationAllowedWithoutInstruction, false);
assert.equal(db.companyPcMainCleanConfirmed, true);
assert.equal(db.draftPrChainOrderConfirmed, true);
assert.equal(db.pr203BaseCompareChainRequired, true);
...
assert.equal(summaryDb.phase21_10_summary.totalChecks, 32);
assert.equal(summaryDb.phase21_10_summary.confirmedChecks, 18);
assert.equal(summaryDb.phase21_10_summary.manualReviewRequiredChecks, 9);
assert.equal(summaryDb.phase21_10_summary.blockedChecks, 5);
assert.equal(summaryDb.phase21_10_summary.phase21_9_postPr202ChainStabilityChecklistPreserved, true);
assert.equal(summaryDb.phase21_10_summary.phase21_9_pr203Created, true);
assert.equal(summaryDb.phase21_10_summary.phase21_10_pushAllowedWithoutInstruction, false);
assert.equal(summaryDb.phase21_10_summary.phase21_10_prCreationAllowedWithoutInstruction, false);
assert.equal(summaryDb.phase21_10_summary.autoReadyForReviewAllowed, false);
assert.equal(summaryDb.phase21_10_summary.readyForReviewAllowed, false);
assert.equal(summaryDb.phase21_10_summary.githubPagesSettingChangeAllowed, false);
assert.equal(summaryDb.phase21_10_summary.publicUrlGuidanceAllowed, false);
assert.equal(summaryDb.phase21_10_summary.autoExecution, false);
assert.equal(summaryDb.phase21_10_summary.mergeAllowed, false);
...
assert.ok(index.includes('id="phase21-10-private-local-post-pr203-operation-checklist-builder"'));
assert.ok(index.includes('<script src="phase21-10-private-local-post-pr203-operation-checklist-builder.js"></script>'));
assert.ok(privateLocal.includes('href="index.html#phase21-10-private-local-post-pr203-operation-checklist-builder"'));
assert.ok(readme.includes("Phase21-10 Private Local Post PR203 Operation Checklist"));
assert.ok(css.includes(".phase21-10-private-local-post-pr203-operation-checklist"));

const builderSource = readText("phase21-10-private-local-post-pr203-operation-checklist-builder.js");
...
for (const forbidden of [
  ...
  assert.equal(builderSource.includes(forbidden), false, `${forbidden} must not appear in Phase21-10 builder`);
}

console.log("phase21-10 private local post PR203 operation checklist builder tests passed");
