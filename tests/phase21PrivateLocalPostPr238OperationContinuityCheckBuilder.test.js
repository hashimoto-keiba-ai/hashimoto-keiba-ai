...
const builder = require("../phase21-44-private-local-post-pr238-operation-continuity-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-44-private-local-post-pr238-operation-continuity-check-db.json");
const summary = readJson("phase21-44-private-local-post-pr238-operation-continuity-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-44-private-local-post-pr238-operation-continuity-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-44");
assert.strictEqual(db.phase, "Phase21-44");
assert.strictEqual(summary.status, "PHASE21_44_PRIVATE_LOCAL_POST_PR238_OPERATION_CONTINUITY_CHECK_READY");
assert.strictEqual(db.baseline, "PR #238 merged main / Phase21-43 reflected");
assert.strictEqual(db.pr238MergedConfirmationRequired, true);
assert.strictEqual(db.phase2143MainReflectionRequired, true);
...
assert.strictEqual(db.operationContinuityConfirmed, true);
...
const panel = builder.buildPrivateLocalPostPr238OperationContinuityCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, "PR #238 merged main / Phase21-43 reflected");
...
assert.ok(panel.records.some((record) => String(record.label).includes("PR #238")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostPr238OperationContinuityCheckPanel"));
assert.ok(index.includes('id="phase21-44-private-local-post-pr238-operation-continuity-check"'));
assert.ok(index.includes('<script src="phase21-44-private-local-post-pr238-operation-continuity-check-builder.js"></script>'));
assert.ok(privateLocal.includes('href="index.html#phase21-44-private-local-post-pr238-operation-continuity-check"'));
assert.ok(readme.includes("Phase21-44"));
assert.ok(readme.includes("PR #238"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-44-private-local-post-pr238-operation-continuity-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostPr238OperationContinuityCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #238")));
assert.ok(rendered.some((line) => line.includes("No auto publish")));
assert.ok(rendered.some((line) => line.includes("public Pages route")));

console.log("Phase21-44 private local post PR238 operation continuity check builder test passed");
