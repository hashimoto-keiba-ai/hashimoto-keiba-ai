...
assert.strictEqual(engine.formatBytes(2048), cleanupEngine.formatBytes(2048), "Phase22-2 uses shared cleanup formatter");
...
assert.strictEqual(engine.isPhase21CleanupKey(engine.RACE_INPUT_STORAGE_KEY), false, "Phase22-1 save key is protected from cleanup");
assert.strictEqual(engine.isPhase21CleanupKey(engine.STORAGE_KEY), false, "Phase22-2 save key is protected from cleanup");
...
assert.strictEqual(engine.isPhase21CleanupKey("otherPhase22Data"), false, "Phase21以外の保存データは対象外");

const cleanupStorage = createStorage();
cleanupStorage.setItem(engine.RACE_INPUT_STORAGE_KEY, JSON.stringify(rawRaceInput));
cleanupStorage.setItem(engine.STORAGE_KEY, JSON.stringify(validPayload));
...
assert.strictEqual(cleanupSummary.count, 2, "Phase22-2 cleanup targets only matching Phase21 keys");
assert.ok(cleanupSummary.bytes > 0, "Phase22-2 cleanup estimates size");
const cleanupBlocked = engine.cleanupPhase21LocalData(cleanupStorage, () => false);
assert.strictEqual(cleanupBlocked.deleted, false, "Phase22-2 cleanup requires confirmation");
...
assert.strictEqual(cleanupDone.deleted, true, "Phase22-2 cleanup runs after confirmation");
...
assert.ok(cleanupStorage.getItem("phase20-final-checklist"), "Phase21以外の保存データは削除しない");
assert.ok(cleanupStorage.getItem(engine.RACE_INPUT_STORAGE_KEY), "Phase22-1 save key is not removed");
assert.ok(cleanupStorage.getItem(engine.STORAGE_KEY), "Phase22-2 save key is not removed");

const limitedStorage = createStorage();
limitedStorage.setItem(engine.RACE_INPUT_STORAGE_KEY, JSON.stringify(rawRaceInput));
...
assert.strictEqual(engine.saveEvaluation(validPayload, raceInput, limitedStorage).quotaExceeded, true, "Phase21 data can block Phase22-2 save before cleanup");
engine.cleanupPhase21LocalData(limitedStorage, () => true);
assert.strictEqual(engine.saveEvaluation(validPayload, raceInput, limitedStorage).saved, true, "Phase21 cleanup allows Phase22-2 save");
...
function createPanelDocument() {
  ...
  const buttons = {
    ...
    "#phase22-cleanup-phase21-storage-for-prediction": { addEventListener(event, handler) { listeners.cleanup = handler; } }
  };
  const cleanupSummaryNode = { textContent: "", dataset: {} };
  const document = {
    readyState: "complete",
    querySelector(selector) {
      ...
      if (selector === "#phase22-prediction-phase21-cleanup-summary") return cleanupSummaryNode;
  ...
  return { document, message, summary, list, cleanupSummaryNode, listeners };
}
...
assert.ok(panel.cleanupSummaryNode.textContent.includes("削除対象候補"), "Phase22-2 cleanup summary is rendered");
...
assert.ok(index.includes('id="phase22-cleanup-phase21-storage-for-prediction"'));
assert.ok(index.includes('id="phase22-prediction-phase21-cleanup-summary"'));
