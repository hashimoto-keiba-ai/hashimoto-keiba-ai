...
const builder = require("../phase21-38-post-pr232-private-local-operation-stability-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-38-post-pr232-private-local-operation-stability-check-db.json");
const summary = readJson("phase21-38-post-pr232-private-local-operation-stability-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-38-post-pr232-private-local-operation-stability-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-38");
assert.strictEqual(db.phase, "Phase21-38");
assert.strictEqual(summary.status, "PHASE21_38_POST_PR232_PRIVATE_LOCAL_OPERATION_STABILITY_CHECK_READY");
assert.strictEqual(db.baseline, "PR #232 merged main / Phase21-37 reflected");
assert.strictEqual(db.pr232MergedConfirmationRequired, true);
assert.strictEqual(db.phase2137MainReflectionRequired, true);
assert.strictEqual(db.homePcMainLatestConfirmed, true);
assert.strictEqual(db.workingTreeCleanConfirmed, true);
assert.strictEqual(db.oldTemporaryPhase2137StashRemoved, true);
...
const panel = builder.buildPostPr232PrivateLocalOperationStabilityCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, "PR #232 merged main / Phase21-37 reflected");
...
assert.strictEqual(panel.pr232MergedConfirmationRequired, true);
assert.strictEqual(panel.phase2137MainReflectionRequired, true);
assert.strictEqual(panel.homePcMainLatestConfirmed, true);
assert.strictEqual(panel.workingTreeCleanConfirmed, true);
assert.strictEqual(panel.oldTemporaryPhase2137StashRemoved, true);
...
assert.ok(panel.stabilityPolicy.includes("Home PC"));
assert.ok(panel.stashPolicy.includes("unrelated stashes remain untouched"));
assert.ok(panel.securitySoftwarePolicy.includes("Norton"));
assert.ok(panel.records.some((record) => String(record.label).includes("PR #232")));
assert.ok(panel.records.some((record) => String(record.label).includes("Phase21-37")));
...
assert.ok(panel.records.some((record) => String(record.label).includes("temporary Phase21-37 stash")));
...
assert.ok(builderSource.includes("runAndRenderPostPr232PrivateLocalOperationStabilityCheckPanel"));
assert.ok(index.includes('id="phase21-38-post-pr232-private-local-operation-stability-check"'));
assert.ok(index.includes('<script src="phase21-38-post-pr232-private-local-operation-stability-check-builder.js"></script>'));
assert.ok(privateLocal.includes('href="index.html#phase21-38-post-pr232-private-local-operation-stability-check"'));
assert.ok(readme.includes("Phase21-38"));
assert.ok(readme.includes("PR #232"));
assert.ok(readme.includes("Phase21-37"));
assert.ok(readme.includes("Home PC"));
assert.ok(readme.includes("working tree clean"));
assert.ok(readme.includes("old temporary Phase21-37 stash removed"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-38-post-pr232-private-local-operation-stability-check-list") {
      return {
        textContent: "old",
...
builder.renderPostPr232PrivateLocalOperationStabilityCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #232")));
...
console.log("Phase21-38 post PR232 private local operation stability check builder test passed");
