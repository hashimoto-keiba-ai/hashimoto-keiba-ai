...
const builder = require("../phase21-52-private-local-post-pr246-stacked-continuation-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-52-private-local-post-pr246-stacked-continuation-check-db.json");
const summary = readJson("phase21-52-private-local-post-pr246-stacked-continuation-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-52-private-local-post-pr246-stacked-continuation-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-52");
assert.strictEqual(builder.PANEL_STATUS, "phase21_52_private_local_post_pr246_stacked_continuation_check_plan_only");
assert.strictEqual(db.phase, "Phase21-52");
assert.strictEqual(summary.status, "PHASE21_52_PRIVATE_LOCAL_POST_PR246_STACKED_CONTINUATION_CHECK_READY");
assert.strictEqual(db.baseline, "Draft PR #243, Draft PR #244, Draft PR #245, and Draft PR #246 remain Draft / PR #246 base is Phase21-50 branch");
...
const panel = builder.buildPrivateLocalPostPr246StackedContinuationCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, "Draft PR #243, Draft PR #244, Draft PR #245, and Draft PR #246 remain Draft / PR #246 base is Phase21-50 branch");
...
assert.ok(panel.japaneseTextContinuityPolicy.includes("Phase21-51"));
...
assert.ok(panel.records.some((record) => String(record.label).includes("Phase21-50 branch")));
assert.ok(panel.records.some((record) => String(record.label).includes("Phase21-51")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostPr246StackedContinuationCheckPanel"));
assert.ok(index.includes('id="phase21-52-private-local-post-pr246-stacked-continuation-check"'));
assert.ok(index.includes('<script src="phase21-52-private-local-post-pr246-stacked-continuation-check-builder.js"></script>'));
assert.ok(index.includes("PR #246 is confirmed as based on the Phase21-50 branch"));
assert.ok(privateLocal.includes('href="index.html#phase21-52-private-local-post-pr246-stacked-continuation-check"'));
assert.ok(readme.includes("Phase21-52"));
assert.ok(readme.includes("Draft PR #246"));
assert.ok(readme.includes("Phase21-50 branch"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-52-private-local-post-pr246-stacked-continuation-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostPr246StackedContinuationCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 35);
assert.ok(rendered.some((line) => line.includes("Draft PR #246 remains Draft")));
assert.ok(rendered.some((line) => line.includes("Phase21-50 branch")));
...
console.log("Phase21-52 private local post PR246 stacked continuation check builder test passed");
