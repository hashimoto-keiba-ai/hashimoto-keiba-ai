...
const builder = require("../phase21-56-private-local-home-pc-continuation-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-56-private-local-home-pc-continuation-check-db.json");
const summary = readJson("phase21-56-private-local-home-pc-continuation-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-56-private-local-home-pc-continuation-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-56");
assert.strictEqual(builder.PANEL_STATUS, "phase21_56_private_local_home_pc_continuation_check_plan_only");
assert.strictEqual(db.phase, "Phase21-56");
assert.strictEqual(summary.status, "PHASE21_56_PRIVATE_LOCAL_HOME_PC_CONTINUATION_CHECK_READY");
assert.strictEqual(db.latestMainHead, "9ba162c");
assert.ok(db.baseline.includes("PR #250 merged"));
assert.ok(db.baseline.includes("home PC development resumed"));
assert.strictEqual(db.previousPhase, "Phase21-55");
assert.strictEqual(db.previousPullRequest, "PR #250");
assert.strictEqual(db.homePcDevelopmentResumed, true);
assert.strictEqual(db.mainSyncedAfterPr250, true);
assert.strictEqual(db.phase2155MainReflectionConfirmed, true);
...
const panel = builder.buildPrivateLocalHomePcContinuationCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, db.baseline);
assert.strictEqual(panel.latestMainHead, "9ba162c");
assert.strictEqual(panel.homePcDevelopmentResumed, true);
assert.strictEqual(panel.phase2155MainReflectionConfirmed, true);
...
assert.ok(panel.records.some((record) => String(record.label).includes("PR #250 merged")));
assert.ok(panel.records.some((record) => String(record.label).includes("Latest main HEAD is 9ba162c")));
assert.ok(panel.records.some((record) => String(record.label).includes("Home PC development resumed")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalHomePcContinuationCheckPanel"));
assert.ok(index.includes('id="phase21-56-private-local-home-pc-continuation-check"'));
assert.ok(index.includes('<script src="phase21-56-private-local-home-pc-continuation-check-builder.js"></script>'));
assert.ok(index.indexOf("phase21-56-private-local-home-pc-continuation-check") < index.indexOf("phase21-55-private-local-post-final-stack-continuation-check"));
assert.ok(privateLocal.includes('href="index.html#phase21-56-private-local-home-pc-continuation-check"'));
assert.ok(privateLocal.indexOf("phase21-56-private-local-home-pc-continuation-check") < privateLocal.indexOf("phase21-55-private-local-post-final-stack-continuation-check"));
assert.ok(readme.includes("Phase21-56"));
assert.ok(readme.includes("PR #250 merged"));
assert.ok(readme.includes("home PC development resumed"));
assert.ok(readme.includes("9ba162c"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-56-private-local-home-pc-continuation-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalHomePcContinuationCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #250 merged")));
assert.ok(rendered.some((line) => line.includes("9ba162c")));
assert.ok(rendered.some((line) => line.includes("No suspicious auto-run script")));

console.log("Phase21-56 private local home PC continuation check builder test passed");
