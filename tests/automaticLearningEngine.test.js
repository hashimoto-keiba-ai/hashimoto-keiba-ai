...
assert.equal(engine.VERSION, '2.6', '自動学習AIのバージョンが2.6である');
...
Object.values(engine.STORAGE_KEYS).forEach((key) => {
  ...
  assert.equal(saved.version, '2.6', `${key}にv2.6として保存する`);
...
assert.equal(status.returnRateImprovement, 19, '回収率改善値をbefore/after差分から計算する');
...
assert.ok(engine.EVOLUTION_HISTORY.includes('v2.6 自己進化エンジン'), 'AI進化履歴にv2.6を含める');
