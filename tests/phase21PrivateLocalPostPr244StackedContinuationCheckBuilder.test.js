...
const builder = require("../phase21-50-private-local-post-pr244-stacked-continuation-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-50-private-local-post-pr244-stacked-continuation-check-db.json");
const summary = readJson("phase21-50-private-local-post-pr244-stacked-continuation-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-50-private-local-post-pr244-stacked-continuation-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-50");
assert.strictEqual(builder.PANEL_STATUS, "phase21_50_private_local_post_pr244_stacked_continuation_check_plan_only");
assert.strictEqual(db.phase, "Phase21-50");
assert.strictEqual(summary.status, "PHASE21_50_PRIVATE_LOCAL_POST_PR244_STACKED_CONTINUATION_CHECK_READY");
assert.strictEqual(db.baseline, "Draft PR #243 and Draft PR #244 remain Draft / PR #244 base is Phase21-48 branch");
...
const panel = builder.buildPrivateLocalPostPr244StackedContinuationCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, "Draft PR #243 and Draft PR #244 remain Draft / PR #244 base is Phase21-48 branch");
...
assert.ok(panel.japaneseTextContinuityPolicy.includes("Phase21-49"));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostPr244StackedContinuationCheckPanel"));
assert.ok(index.includes('id="phase21-50-private-local-post-pr244-stacked-continuation-check"'));
assert.ok(index.includes('<script src="phase21-50-private-local-post-pr244-stacked-continuation-check-builder.js"></script>'));
assert.ok(index.includes("PR #244 is confirmed as based on the Phase21-48 branch"));
assert.ok(privateLocal.includes('href="index.html#phase21-50-private-local-post-pr244-stacked-continuation-check"'));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-50-private-local-post-pr244-stacked-continuation-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostPr244StackedContinuationCheckPanel(panel, fakeDoc);
...
console.log("Phase21-50 private local post PR244 stacked continuation check builder test passed");
