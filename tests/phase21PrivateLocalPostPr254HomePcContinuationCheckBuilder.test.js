...
const builder = require("../phase21-60-private-local-post-pr254-home-pc-continuation-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-60-private-local-post-pr254-home-pc-continuation-check-db.json");
const summary = readJson("phase21-60-private-local-post-pr254-home-pc-continuation-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-60-private-local-post-pr254-home-pc-continuation-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-60");
assert.strictEqual(builder.PANEL_STATUS, "phase21_60_private_local_post_pr254_home_pc_continuation_check_plan_only");
assert.strictEqual(db.phase, "Phase21-60");
assert.strictEqual(summary.status, "PHASE21_60_PRIVATE_LOCAL_POST_PR254_HOME_PC_CONTINUATION_CHECK_READY");
assert.strictEqual(db.latestMainHead, "edcc536");
assert.ok(db.baseline.includes("PR #254 merged"));
assert.ok(db.baseline.includes("home PC main updated / private-local.html confirmed"));
assert.strictEqual(db.previousPhase, "Phase21-59");
assert.strictEqual(db.previousPullRequest, "PR #254");
assert.strictEqual(db.homePcMainUpdatedAndPrivateLocalHtmlConfirmed, true);
assert.strictEqual(db.mainSyncedAfterPr254, true);
assert.strictEqual(db.phase2159MainReflectionConfirmed, true);
...
const panel = builder.buildPrivateLocalPostPr254HomePcContinuationCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, db.baseline);
assert.strictEqual(panel.latestMainHead, "edcc536");
assert.strictEqual(panel.homePcMainUpdatedAndPrivateLocalHtmlConfirmed, true);
assert.strictEqual(panel.phase2159MainReflectionConfirmed, true);
...
assert.ok(panel.records.some((record) => String(record.label).includes("PR #254 merged")));
assert.ok(panel.records.some((record) => String(record.label).includes("Latest main HEAD is edcc536")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostPr254HomePcContinuationCheckPanel"));
assert.ok(index.includes('id="phase21-60-private-local-post-pr254-home-pc-continuation-check"'));
assert.ok(index.includes('<script src="phase21-60-private-local-post-pr254-home-pc-continuation-check-builder.js"></script>'));
assert.ok(index.indexOf("phase21-60-private-local-post-pr254-home-pc-continuation-check") < index.indexOf("phase21-59-private-local-post-pr253-home-pc-continuation-check"));
assert.ok(privateLocal.includes('href="index.html#phase21-60-private-local-post-pr254-home-pc-continuation-check"'));
assert.ok(privateLocal.indexOf("phase21-60-private-local-post-pr254-home-pc-continuation-check") < privateLocal.indexOf("phase21-59-private-local-post-pr253-home-pc-continuation-check"));
assert.ok(readme.includes("Phase21-60"));
assert.ok(readme.includes("PR #254 merged"));
assert.ok(readme.includes("home PC main updated / private-local.html confirmed"));
assert.ok(readme.includes("edcc536"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-60-private-local-post-pr254-home-pc-continuation-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostPr254HomePcContinuationCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #254 merged")));
assert.ok(rendered.some((line) => line.includes("edcc536")));
assert.ok(rendered.some((line) => line.includes("No suspicious auto-run script")));

console.log("Phase21-60 private local post PR254 home PC continuation check builder test passed");
