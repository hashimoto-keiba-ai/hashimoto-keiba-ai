...
const builder = require("../phase21-39-post-pr233-private-local-operation-continuity-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-39-post-pr233-private-local-operation-continuity-check-db.json");
const summary = readJson("phase21-39-post-pr233-private-local-operation-continuity-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-39-post-pr233-private-local-operation-continuity-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-39");
assert.strictEqual(db.phase, "Phase21-39");
assert.strictEqual(summary.status, "PHASE21_39_POST_PR233_PRIVATE_LOCAL_OPERATION_CONTINUITY_CHECK_READY");
assert.strictEqual(db.baseline, "PR #233 merged main / Phase21-38 reflected");
assert.strictEqual(db.pr233MergedConfirmationRequired, true);
assert.strictEqual(db.phase2138MainReflectionRequired, true);
assert.strictEqual(db.homePcMainLatestConfirmed, true);
assert.strictEqual(db.workingTreeCleanConfirmed, true);
assert.strictEqual(db.phase2139StartsFromCleanMain, true);
...
const panel = builder.buildPostPr233PrivateLocalOperationContinuityCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, "PR #233 merged main / Phase21-38 reflected");
...
assert.strictEqual(panel.pr233MergedConfirmationRequired, true);
assert.strictEqual(panel.phase2138MainReflectionRequired, true);
assert.strictEqual(panel.homePcMainLatestConfirmed, true);
assert.strictEqual(panel.workingTreeCleanConfirmed, true);
assert.strictEqual(panel.phase2139StartsFromCleanMain, true);
...
assert.ok(panel.continuityPolicy.includes("Home PC"));
...
assert.ok(panel.records.some((record) => String(record.label).includes("PR #233")));
assert.ok(panel.records.some((record) => String(record.label).includes("Phase21-38")));
...
assert.ok(panel.records.some((record) => String(record.label).includes("Phase21-39 starts from clean main")));
...
assert.ok(builderSource.includes("runAndRenderPostPr233PrivateLocalOperationContinuityCheckPanel"));
assert.ok(index.includes('id="phase21-39-post-pr233-private-local-operation-continuity-check"'));
assert.ok(index.includes('<script src="phase21-39-post-pr233-private-local-operation-continuity-check-builder.js"></script>'));
assert.ok(privateLocal.includes('href="index.html#phase21-39-post-pr233-private-local-operation-continuity-check"'));
assert.ok(readme.includes("Phase21-39"));
assert.ok(readme.includes("PR #233"));
assert.ok(readme.includes("Phase21-38"));
assert.ok(readme.includes("Home PC"));
assert.ok(readme.includes("working tree clean"));
assert.ok(readme.includes("Phase21-39 starts from clean main"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-39-post-pr233-private-local-operation-continuity-check-list") {
      return {
        textContent: "old",
...
builder.renderPostPr233PrivateLocalOperationContinuityCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #233")));
...
console.log("Phase21-39 post PR233 private local operation continuity check builder test passed");
