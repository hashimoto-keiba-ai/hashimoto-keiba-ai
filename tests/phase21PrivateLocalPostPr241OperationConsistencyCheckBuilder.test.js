...
const builder = require("../phase21-47-private-local-post-pr241-operation-consistency-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-47-private-local-post-pr241-operation-consistency-check-db.json");
const summary = readJson("phase21-47-private-local-post-pr241-operation-consistency-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-47-private-local-post-pr241-operation-consistency-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-47");
assert.strictEqual(db.phase, "Phase21-47");
assert.strictEqual(summary.status, "PHASE21_47_PRIVATE_LOCAL_POST_PR241_OPERATION_CONSISTENCY_CHECK_READY");
...
assert.strictEqual(db.operationConsistencyConfirmed, true);
...
const panel = builder.buildPrivateLocalPostPr241OperationConsistencyCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostPr241OperationConsistencyCheckPanel"));
assert.ok(index.includes('id="phase21-47-private-local-post-pr241-operation-consistency-check"'));
assert.ok(index.includes('<script src="phase21-47-private-local-post-pr241-operation-consistency-check-builder.js"></script>'));
assert.ok(privateLocal.includes('href="index.html#phase21-47-private-local-post-pr241-operation-consistency-check"'));
assert.ok(readme.includes("Phase21-47"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-47-private-local-post-pr241-operation-consistency-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostPr241OperationConsistencyCheckPanel(panel, fakeDoc);
...
console.log("Phase21-47 private local post PR241 operation consistency check builder test passed");
