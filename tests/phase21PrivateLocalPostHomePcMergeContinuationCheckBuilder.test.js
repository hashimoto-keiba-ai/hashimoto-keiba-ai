...
const builder = require("../phase21-57-private-local-post-home-pc-merge-continuation-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-57-private-local-post-home-pc-merge-continuation-check-db.json");
const summary = readJson("phase21-57-private-local-post-home-pc-merge-continuation-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-57-private-local-post-home-pc-merge-continuation-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-57");
assert.strictEqual(builder.PANEL_STATUS, "phase21_57_private_local_post_home_pc_merge_continuation_check_plan_only");
assert.strictEqual(db.phase, "Phase21-57");
assert.strictEqual(summary.status, "PHASE21_57_PRIVATE_LOCAL_POST_HOME_PC_MERGE_CONTINUATION_CHECK_READY");
assert.strictEqual(db.latestMainHead, "0816f88");
assert.ok(db.baseline.includes("PR #251 merged"));
assert.ok(db.baseline.includes("home PC main updated"));
assert.strictEqual(db.previousPhase, "Phase21-56");
assert.strictEqual(db.previousPullRequest, "PR #251");
assert.strictEqual(db.homePcMainUpdated, true);
assert.strictEqual(db.mainSyncedAfterPr251, true);
assert.strictEqual(db.phase2156MainReflectionConfirmed, true);
...
const panel = builder.buildPrivateLocalPostHomePcMergeContinuationCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, db.baseline);
assert.strictEqual(panel.latestMainHead, "0816f88");
assert.strictEqual(panel.homePcMainUpdated, true);
assert.strictEqual(panel.phase2156MainReflectionConfirmed, true);
...
assert.ok(panel.records.some((record) => String(record.label).includes("PR #251 merged")));
assert.ok(panel.records.some((record) => String(record.label).includes("Latest main HEAD is 0816f88")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostHomePcMergeContinuationCheckPanel"));
assert.ok(index.includes('id="phase21-57-private-local-post-home-pc-merge-continuation-check"'));
assert.ok(index.includes('<script src="phase21-57-private-local-post-home-pc-merge-continuation-check-builder.js"></script>'));
assert.ok(index.indexOf("phase21-57-private-local-post-home-pc-merge-continuation-check") < index.indexOf("phase21-56-private-local-home-pc-continuation-check"));
assert.ok(privateLocal.includes('href="index.html#phase21-57-private-local-post-home-pc-merge-continuation-check"'));
assert.ok(privateLocal.indexOf("phase21-57-private-local-post-home-pc-merge-continuation-check") < privateLocal.indexOf("phase21-56-private-local-home-pc-continuation-check"));
assert.ok(readme.includes("Phase21-57"));
assert.ok(readme.includes("PR #251 merged"));
assert.ok(readme.includes("home PC main updated"));
assert.ok(readme.includes("0816f88"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-57-private-local-post-home-pc-merge-continuation-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostHomePcMergeContinuationCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #251 merged")));
assert.ok(rendered.some((line) => line.includes("0816f88")));
assert.ok(rendered.some((line) => line.includes("No suspicious auto-run script")));

console.log("Phase21-57 private local post home PC merge continuation check builder test passed");
