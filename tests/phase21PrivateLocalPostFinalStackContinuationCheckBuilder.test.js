...
const builder = require("../phase21-55-private-local-post-final-stack-continuation-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-55-private-local-post-final-stack-continuation-check-db.json");
const summary = readJson("phase21-55-private-local-post-final-stack-continuation-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-55-private-local-post-final-stack-continuation-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-55");
assert.strictEqual(builder.PANEL_STATUS, "phase21_55_private_local_post_final_stack_continuation_check_plan_only");
assert.strictEqual(db.phase, "Phase21-55");
assert.strictEqual(summary.status, "PHASE21_55_PRIVATE_LOCAL_POST_FINAL_STACK_CONTINUATION_CHECK_READY");
assert.strictEqual(db.latestMainHead, "ccfd4a9");
assert.ok(db.baseline.includes("PR #249 merged"));
assert.ok(db.baseline.includes("company PC resumed"));
assert.strictEqual(db.previousPhase, "Phase21-54");
assert.strictEqual(db.previousPullRequest, "PR #249");
assert.strictEqual(db.companyPcResumed, true);
assert.strictEqual(db.mainSyncedAfterPr249, true);
assert.strictEqual(db.phase2154MainReflectionConfirmed, true);
...
const panel = builder.buildPrivateLocalPostFinalStackContinuationCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, db.baseline);
assert.strictEqual(panel.latestMainHead, "ccfd4a9");
assert.strictEqual(panel.companyPcResumed, true);
assert.strictEqual(panel.phase2154MainReflectionConfirmed, true);
...
assert.ok(panel.safeLocalUsePolicy.includes("company PC main sync"));
assert.ok(panel.prPolicy.includes("draft PR only"));
assert.ok(panel.records.some((record) => String(record.label).includes("PR #249 merged")));
assert.ok(panel.records.some((record) => String(record.label).includes("Latest main HEAD is ccfd4a9")));
assert.ok(panel.records.some((record) => String(record.label).includes("Company PC resumed")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostFinalStackContinuationCheckPanel"));
assert.ok(index.includes('id="phase21-55-private-local-post-final-stack-continuation-check"'));
assert.ok(index.includes('<script src="phase21-55-private-local-post-final-stack-continuation-check-builder.js"></script>'));
assert.ok(index.indexOf("phase21-55-private-local-post-final-stack-continuation-check") < index.indexOf("phase21-54-private-local-final-stack-merge-preparation-check"));
assert.ok(privateLocal.includes('href="index.html#phase21-55-private-local-post-final-stack-continuation-check"'));
assert.ok(privateLocal.indexOf("phase21-55-private-local-post-final-stack-continuation-check") < privateLocal.indexOf("phase21-54-private-local-final-stack-merge-preparation-check"));
assert.ok(readme.includes("Phase21-55"));
assert.ok(readme.includes("PR #249 merged"));
assert.ok(readme.includes("company PC resumed"));
assert.ok(readme.includes("ccfd4a9"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-55-private-local-post-final-stack-continuation-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostFinalStackContinuationCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #249 merged")));
assert.ok(rendered.some((line) => line.includes("ccfd4a9")));
assert.ok(rendered.some((line) => line.includes("No suspicious auto-run script")));

console.log("Phase21-55 private local post final stack continuation check builder test passed");
