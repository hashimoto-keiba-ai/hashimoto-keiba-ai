...
const builder = require("../phase21-53-private-local-post-pr247-stacked-continuation-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-53-private-local-post-pr247-stacked-continuation-check-db.json");
const summary = readJson("phase21-53-private-local-post-pr247-stacked-continuation-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-53-private-local-post-pr247-stacked-continuation-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-53");
assert.strictEqual(builder.PANEL_STATUS, "phase21_53_private_local_post_pr247_stacked_continuation_check_plan_only");
assert.strictEqual(db.phase, "Phase21-53");
assert.strictEqual(summary.status, "PHASE21_53_PRIVATE_LOCAL_POST_PR247_STACKED_CONTINUATION_CHECK_READY");
assert.strictEqual(db.baseline, "Draft PR #243, Draft PR #244, Draft PR #245, Draft PR #246, and Draft PR #247 remain Draft / PR #247 base is Phase21-51 branch");
...
assert.strictEqual(db.draftPr246RemainsDraft, true);
assert.strictEqual(db.draftPr247RemainsDraft, true);
assert.strictEqual(db.pr244BasePhase2148BranchConfirmed, true);
assert.strictEqual(db.pr245BasePhase2149BranchConfirmed, true);
assert.strictEqual(db.pr246BasePhase2150BranchConfirmed, true);
assert.strictEqual(db.pr247BasePhase2151BranchConfirmed, true);
...
assert.strictEqual(db.phase2152FlowMaintained, true);
assert.strictEqual(db.phase2151FlowMaintained, true);
...
const panel = builder.buildPrivateLocalPostPr247StackedContinuationCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, "Draft PR #243, Draft PR #244, Draft PR #245, Draft PR #246, and Draft PR #247 remain Draft / PR #247 base is Phase21-51 branch");
...
assert.strictEqual(panel.draftPr246RemainsDraft, true);
assert.strictEqual(panel.draftPr247RemainsDraft, true);
...
assert.strictEqual(panel.pr246BasePhase2150BranchConfirmed, true);
assert.strictEqual(panel.pr247BasePhase2151BranchConfirmed, true);
...
assert.ok(panel.japaneseTextContinuityPolicy.includes("Phase21-52"));
...
assert.ok(panel.records.some((record) => String(record.label).includes("Draft PR #246 remains Draft")));
assert.ok(panel.records.some((record) => String(record.label).includes("Draft PR #247 remains Draft")));
...
assert.ok(panel.records.some((record) => String(record.label).includes("Phase21-51 branch")));
assert.ok(panel.records.some((record) => String(record.label).includes("Phase21-52")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostPr247StackedContinuationCheckPanel"));
assert.ok(index.includes('id="phase21-53-private-local-post-pr247-stacked-continuation-check"'));
assert.ok(index.includes('<script src="phase21-53-private-local-post-pr247-stacked-continuation-check-builder.js"></script>'));
assert.ok(index.includes("PR #247 is confirmed as based on the Phase21-51 branch"));
assert.ok(privateLocal.includes('href="index.html#phase21-53-private-local-post-pr247-stacked-continuation-check"'));
assert.ok(readme.includes("Phase21-53"));
assert.ok(readme.includes("Draft PR #247"));
assert.ok(readme.includes("Phase21-51 branch"));
assert.ok(readme.includes("Phase21-52"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-53-private-local-post-pr247-stacked-continuation-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostPr247StackedContinuationCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 35);
assert.ok(rendered.some((line) => line.includes("Draft PR #247 remains Draft")));
assert.ok(rendered.some((line) => line.includes("Phase21-51 branch")));
...
console.log("Phase21-53 private local post PR247 stacked continuation check builder test passed");
