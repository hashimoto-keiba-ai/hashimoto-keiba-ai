...
const builder = require("../phase21-46-private-local-post-pr240-operation-integrity-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-46-private-local-post-pr240-operation-integrity-check-db.json");
const summary = readJson("phase21-46-private-local-post-pr240-operation-integrity-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-46-private-local-post-pr240-operation-integrity-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-46");
assert.strictEqual(db.phase, "Phase21-46");
assert.strictEqual(summary.status, "PHASE21_46_PRIVATE_LOCAL_POST_PR240_OPERATION_INTEGRITY_CHECK_READY");
assert.strictEqual(db.baseline, "PR #240 merged main / Phase21-45 reflected");
assert.strictEqual(db.pr240MergedConfirmationRequired, true);
assert.strictEqual(db.phase2145MainReflectionRequired, true);
...
assert.strictEqual(db.operationIntegrityConfirmed, true);
...
const panel = builder.buildPrivateLocalPostPr240OperationIntegrityCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, "PR #240 merged main / Phase21-45 reflected");
...
assert.ok(panel.records.some((record) => String(record.label).includes("PR #240")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostPr240OperationIntegrityCheckPanel"));
assert.ok(index.includes('id="phase21-46-private-local-post-pr240-operation-integrity-check"'));
assert.ok(index.includes('<script src="phase21-46-private-local-post-pr240-operation-integrity-check-builder.js"></script>'));
assert.ok(privateLocal.includes('href="index.html#phase21-46-private-local-post-pr240-operation-integrity-check"'));
assert.ok(readme.includes("Phase21-46"));
assert.ok(readme.includes("PR #240"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-46-private-local-post-pr240-operation-integrity-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostPr240OperationIntegrityCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #240")));
assert.ok(rendered.some((line) => line.includes("No auto publish")));
assert.ok(rendered.some((line) => line.includes("public Pages route")));

console.log("Phase21-46 private local post PR240 operation integrity check builder test passed");
