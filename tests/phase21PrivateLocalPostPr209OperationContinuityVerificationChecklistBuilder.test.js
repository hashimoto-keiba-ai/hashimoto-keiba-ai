...
const engine = require("../phase21-16-private-local-post-pr209-operation-continuity-verification-checklist-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-16-private-local-post-pr209-operation-continuity-verification-checklist-db.json");
const summaryDb = readJson("phase21-16-private-local-post-pr209-operation-continuity-verification-checklist-summary-db.json");
...
assert.equal(engine.PHASE, "Phase21-16");
assert.equal(engine.CHECKLIST_NAME, "Private Local Post PR209 Operation Continuity Verification Checklist");
assert.equal(engine.EXECUTION_POLICY, "PLAN_ONLY");
assert.equal(engine.PROTECTION_POLICY, "Protected");
assert.equal(engine.PANEL_STATUS, "phase21_16_private_local_post_pr209_operation_continuity_verification_checklist_plan_only");
assert.equal(engine.POST_PR209_OPERATION_CONTINUITY_VERIFICATION_STATUS, "POST_PR209_PRIVATE_LOCAL_OPERATION_CONTINUITY_VERIFICATION_READY");
...
const panel = engine.buildPrivateLocalPostPr209OperationContinuityVerificationChecklist({}, () => new Date("2026-07-03T00:00:00.000Z"));
assert.equal(panel.phase, "Phase21-16");
...
assert.equal(panel.post_pr209_operation_continuity_verification_checklist_only, true);
assert.equal(panel.phase21_14_post_pr207_operation_stability_continuation_checklist_preserved, true);
assert.equal(panel.phase21_15_post_pr208_operation_continuity_stabilization_checklist_preserved, true);
assert.equal(panel.pr209_merged_to_main, true);
assert.equal(panel.main_latest_after_pr209_merge, true);
...
assert.equal(panel.phase21_16_push_explicitly_requested, false);
assert.equal(panel.phase21_16_push_allowed_without_instruction, false);
assert.equal(panel.phase21_16_pr_creation_allowed_without_instruction, false);
...
assert.equal(summary.phase21_15_pr209_merged, true);
assert.equal(summary.pr209_main_merge_confirmed, true);
...
assert.equal(summary.phase21_16_push_explicitly_requested, false);
assert.equal(summary.phase21_16_push_allowed_without_instruction, false);
assert.equal(summary.phase21_16_pr_creation_allowed_without_instruction, false);
...
assert.equal(db.phase, "Phase21-16");
assert.equal(db.postPr209OperationContinuityVerificationStatus, "POST_PR209_PRIVATE_LOCAL_OPERATION_CONTINUITY_VERIFICATION_READY");
assert.equal(db.pr209MergedToMain, true);
assert.equal(db.mainLatestAfterPr209Merge, true);
...
assert.equal(db.phase21_15_mergedPr, "PR #209 Merged");
assert.equal(db.phase21_15_postPr208OperationContinuityStabilizationChecklistPreserved, true);
...
assert.equal(db.phase21_16_pushExplicitlyRequested, false);
assert.equal(db.phase21_16_prCreationAllowedWithoutInstruction, false);
...
assert.equal(summaryDb.phase21_16_summary.totalChecks, 33);
assert.equal(summaryDb.phase21_16_summary.phase21_14_postPr207OperationStabilityContinuationChecklistPreserved, true);
assert.equal(summaryDb.phase21_16_summary.phase21_15_mergedPr, "PR #209 Merged");
assert.equal(summaryDb.phase21_16_summary.phase21_15_postPr208OperationContinuityStabilizationChecklistPreserved, true);
assert.equal(summaryDb.phase21_16_summary.pr209MergedToMain, true);
assert.equal(summaryDb.phase21_16_summary.localMainOriginMainOriginHeadMatch, true);
assert.equal(summaryDb.phase21_16_summary.workingTreeCleanConfirmed, true);
assert.equal(summaryDb.phase21_16_summary.privateLocalContinuityPrioritized, true);
assert.equal(summaryDb.phase21_16_summary.publicDeliveryChangeAllowed, false);
assert.equal(summaryDb.phase21_16_summary.validationResultsSaveBeforeDraftPrRequired, true);
assert.equal(summaryDb.phase21_16_summary.phase21_16_pushExplicitlyRequested, false);
assert.equal(summaryDb.phase21_16_summary.phase21_16_prCreationAllowedWithoutInstruction, false);
assert.equal(summaryDb.phase21_16_summary.autoReadyForReviewAllowed, false);
assert.equal(summaryDb.phase21_16_summary.readyForReviewAllowed, false);
assert.equal(summaryDb.phase21_16_summary.githubPagesSettingChangeAllowed, false);
assert.equal(summaryDb.phase21_16_summary.publicUrlGuidanceAllowed, false);
assert.equal(summaryDb.phase21_16_summary.autoExecution, false);
assert.equal(summaryDb.phase21_16_summary.mergeAllowed, false);
...
assert.ok(index.includes('id="phase21-16-private-local-post-pr209-operation-continuity-verification-checklist-builder"'));
assert.ok(index.includes('<script src="phase21-16-private-local-post-pr209-operation-continuity-verification-checklist-builder.js"></script>'));
assert.ok(privateLocal.includes('href="index.html#phase21-16-private-local-post-pr209-operation-continuity-verification-checklist-builder"'));
assert.ok(readme.includes("Phase21-16 Private Local Post PR209 Operation Continuity Verification Checklist"));
assert.ok(css.includes(".phase21-16-private-local-post-pr209-operation-continuity-verification-checklist"));

const builderSource = readText("phase21-16-private-local-post-pr209-operation-continuity-verification-checklist-builder.js");
...
for (const forbidden of [
  ...
  assert.equal(builderSource.includes(forbidden), false, `${forbidden} must not appear in Phase21-16 builder`);
}

console.log("phase21-16 private local post PR209 operation continuity verification checklist builder tests passed");
