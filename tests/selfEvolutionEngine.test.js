...
assert.equal(engine.VERSION, '2.8', '自己進化エンジンのバージョンが2.8である');
...
assert.deepEqual(engine.buildEvolutionPipeline(), ['検証', '分析', '学習', '更新', '履歴保存'], '検証→分析→学習→更新→履歴保存の完全自動パイプライン');
const pipeline = engine.buildFullAutoEvolutionPipeline({ storage });
assert.equal(pipeline.engine, 'selfEvolutionEngine', '自己進化AIエンジンを構築する');
assert.ok(pipeline.historyTargets.includes('history-db.json'), '履歴保存先を含める');

const record = engine.saveSelfEvolutionRule({ storage });
assert.equal(record.updateTarget, '三連単パターン', '自己進化ルール保存時に更新対象を保持する');
assert.equal(storage.readJson(engine.STORAGE_KEYS.evolution).version, '2.8', 'self-evolution-dbへ保存する');
