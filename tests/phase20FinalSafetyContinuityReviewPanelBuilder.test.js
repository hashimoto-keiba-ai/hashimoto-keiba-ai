...
const engine = require("../phase20-20-final-safety-continuity-review-panel-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase20-20-final-safety-continuity-review-panel-db.json");
const summaryDb = readJson("phase20-20-final-safety-continuity-review-panel-summary-db.json");
...
assert.equal(engine.PHASE, "Phase20-20");
assert.equal(engine.EXECUTION_POLICY, "PLAN_ONLY");
assert.equal(engine.PROTECTION_POLICY, "Protected");
assert.equal(engine.PANEL_STATUS, "final_safety_continuity_review_plan_only");
...
assert.equal(engine.REVIEW_SCOPE.length, 12);

const panel = engine.buildFinalSafetyContinuityReviewPanel({}, () => new Date("2026-06-29T07:00:00.000Z"));
assert.equal(panel.phase, "Phase20-20");
assert.equal(panel.panel_status, "final_safety_continuity_review_plan_only");
...
assert.deepEqual(panel.phase20_20_summary, {
...
for (const record of panel.records) {
  assert.equal(record.review_status, "final_safety_continuity_confirmed");
...
assert.equal(db.phase, "Phase20-20");
assert.equal(db.panelStatus, "final_safety_continuity_review_plan_only");
...
assert.equal(summaryDb.phase, "Phase20-20");
assert.equal(summaryDb.phase20_20_summary.totalSafetyChecks, 15);
assert.equal(summaryDb.phase20_20_summary.repositoryPrivatePremise, true);
assert.equal(summaryDb.phase20_20_summary.localLaunchPolicy, engine.LOCAL_LAUNCH_POLICY);
assert.equal(summaryDb.phase20_20_summary.githubPagesPublicUrlLaunchAllowed, false);
assert.equal(summaryDb.phase20_20_summary.githubPagesLaunchAllowed, false);
assert.equal(summaryDb.phase20_20_summary.publicUrlLaunchAllowed, false);
assert.equal(summaryDb.phase20_20_summary.autoExecution, false);
assert.equal(summaryDb.phase20_20_summary.autoLaunch, false);
assert.equal(summaryDb.phase20_20_summary.phase20_12_to_19_locks_preserved, true);
assert.equal(summaryDb.phase20_20_summary.finalSafetyReviewSafeToContinue, true);
assert.equal(summaryDb.phase20_20_summary.publicReleaseAllowed, false);
assert.equal(summaryDb.phase20_20_summary.unsafeFlagsCount, 0);
...
for (const summary of [phase2012Summary.phase20_12_summary, phase2013Summary.phase20_13_summary, phase2014Summary.phase20_14_summary, phase2015Summary.phase20_15_summary, phase2016Summary.phase20_16_summary, phase2017Summary.phase20_17_summary, phase2018Summary.phase20_18_summary, phase2019Summary.phase20_19_summary]) {
...
assert.equal(phase2019Summary.phase20_19_summary.phase20_12_to_18_locks_preserved, true);

assert.ok(index.includes('id="phase20-20-final-safety-continuity-review-panel-builder"'));
assert.ok(index.includes('<script src="phase20-20-final-safety-continuity-review-panel-builder.js"></script>'));
assert.ok(privateLocal.includes('href="index.html#phase20-20-final-safety-continuity-review-panel-builder"'));
assert.ok(privateLocal.includes("Phase20-20 Final Safety Continuity Review Panel"));
for (const id of ["phase20-19-local-only-safety-continuity-panel-builder", "phase20-18-local-launch-only-verification-gate-builder", "phase20-17-private-safety-status-dashboard-builder", "phase20-16-private-release-lock-audit-gate-builder", "phase20-15-owner-manual-approval-lock-gate-builder", "phase20-14-public-release-final-block-gate-builder", "phase20-13-manual-device-confirmation-gate-builder", "phase20-12-private-operation-safety-gate-builder"]) {
...
assert.ok(readme.includes("Phase20-20 Final Safety Continuity Review Panel"));
assert.ok(readme.includes("phase20-20-final-safety-continuity-review-panel-builder.js"));

const builderSource = readText("phase20-20-final-safety-continuity-review-panel-builder.js");
for (const forbidden of [
  ...
  assert.equal(builderSource.includes(forbidden), false, `${forbidden} must not appear in Phase20-20 builder`);
}

console.log("phase20-20 final safety continuity review panel builder tests passed");
