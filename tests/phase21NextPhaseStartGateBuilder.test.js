...
const builder = require("../phase21-31-next-phase-start-gate-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-31-next-phase-start-gate-db.json");
const summary = readJson("phase21-31-next-phase-start-gate-summary-db.json");
const builderSource = readText("phase21-31-next-phase-start-gate-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-31");
assert.strictEqual(db.phase, "Phase21-31");
assert.strictEqual(summary.status, "PHASE21_31_NEXT_PHASE_START_GATE_READY");
assert.strictEqual(db.priorPrNumber, 224);
assert.strictEqual(summary.priorPrNumber, 224);
assert.strictEqual(db.priorPhase, "Phase21-30");
assert.strictEqual(db.expectedPriorMergeCommit, "8648f1831371c1c6ea52f59fd68f19f413fae556");
...
const panel = builder.buildNextPhaseStartGatePanel({ db, summary }, () => new Date("2026-07-04T00:00:00.000Z"));
assert.strictEqual(panel.priorPrNumber, 224);
assert.strictEqual(panel.priorPhase, "Phase21-30");
assert.strictEqual(panel.expectedPriorMergeCommit, "8648f1831371c1c6ea52f59fd68f19f413fae556");
...
assert.ok(panel.startGatePolicy.includes("PR #224"));
assert.ok(panel.branchPolicy.includes("main"));
...
assert.ok(panel.records.some((record) => String(record.label).includes("Do not implement directly on main")));

assert.ok(builderSource.includes("fetchJson"));
assert.ok(builderSource.includes("catch (error)"));
assert.ok(builderSource.includes("runAndRenderNextPhaseStartGatePanel"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-31-next-phase-start-gate-list") {
      return {
        textContent: "old",
...
builder.renderNextPhaseStartGatePanel(panel, fakeDoc);
assert.ok(rendered.length >= 10);
assert.ok(rendered.some((line) => line.includes("PR #224")));
assert.ok(rendered.some((line) => line.includes("git pull origin main")));
assert.ok(rendered.some((line) => line.includes("Do not implement directly on main")));

console.log("Phase21-31 next phase start gate builder test passed");
