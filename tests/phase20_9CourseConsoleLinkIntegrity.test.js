...
assert.deepEqual(
  consoleLinks.map((link) => `${link.consoleKey}:${link.href}`),
  integrityDb.links.map((link) => `${link.consoleKey}:${link.href}`)
);
...
const fallbackLinks = engine.applyLinkIntegrityFallback(engine.getConsoleDefinition("win5").links, {
  [missingHref]: { status: "missing", reason: "phase20_9_missing_link_fixture" }
});
...
const fallbackReport = engine.renderConsolePage(fallbackDocument, { search: "?console=win5" }, {
  linkStatusMap: { [missingHref]: { status: "missing" } }
});
