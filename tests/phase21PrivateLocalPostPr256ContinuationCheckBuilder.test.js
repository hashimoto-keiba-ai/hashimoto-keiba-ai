...
const builder = require("../phase21-62-private-local-post-pr256-continuation-check-builder.js");

const root = path.resolve(__dirname, "..");
const readText = (file) => fs.readFileSync(path.join(root, file), "utf8");
const readJson = (file) => JSON.parse(readText(file));

const db = readJson("phase21-62-private-local-post-pr256-continuation-check-db.json");
const summary = readJson("phase21-62-private-local-post-pr256-continuation-check-summary-db.json");
const index = readText("index.html");
const privateLocal = readText("private-local.html");
const readme = readText("README.md");
const builderSource = readText("phase21-62-private-local-post-pr256-continuation-check-builder.js");

assert.strictEqual(builder.PHASE, "Phase21-62");
assert.strictEqual(builder.PANEL_STATUS, "phase21_62_private_local_post_pr256_continuation_check_plan_only");
assert.strictEqual(db.phase, "Phase21-62");
assert.strictEqual(summary.status, "PHASE21_62_PRIVATE_LOCAL_POST_PR256_CONTINUATION_CHECK_READY");
assert.strictEqual(db.latestMainHead, "5b577f6");
assert.ok(db.baseline.includes("PR #256 merged"));
assert.ok(db.baseline.includes("home PC main updated / private-local.html confirmed"));
assert.strictEqual(db.previousPhase, "Phase21-61");
assert.strictEqual(db.previousPullRequest, "PR #256");
assert.strictEqual(db.homePcMainUpdatedAndPrivateLocalHtmlConfirmed, true);
assert.strictEqual(db.mainSyncedAfterPr256, true);
assert.strictEqual(db.phase2161MainReflectionConfirmed, true);
...
const panel = builder.buildPrivateLocalPostPr256ContinuationCheckPanel({ db, summary }, () => new Date("2026-07-05T00:00:00.000Z"));
assert.strictEqual(panel.baseline, db.baseline);
assert.strictEqual(panel.latestMainHead, "5b577f6");
assert.strictEqual(panel.homePcMainUpdatedAndPrivateLocalHtmlConfirmed, true);
assert.strictEqual(panel.phase2161MainReflectionConfirmed, true);
...
assert.ok(panel.records.some((record) => String(record.label).includes("PR #256 merged")));
assert.ok(panel.records.some((record) => String(record.label).includes("Latest main HEAD is 5b577f6")));
...
assert.ok(builderSource.includes("runAndRenderPrivateLocalPostPr256ContinuationCheckPanel"));
assert.ok(index.includes('id="phase21-62-private-local-post-pr256-continuation-check"'));
assert.ok(index.includes('<script src="phase21-62-private-local-post-pr256-continuation-check-builder.js"></script>'));
assert.ok(index.indexOf("phase21-62-private-local-post-pr256-continuation-check") < index.indexOf("phase21-61-private-local-post-pr255-continuation-check"));
assert.ok(privateLocal.includes('href="index.html#phase21-62-private-local-post-pr256-continuation-check"'));
assert.ok(privateLocal.indexOf("phase21-62-private-local-post-pr256-continuation-check") < privateLocal.indexOf("phase21-61-private-local-post-pr255-continuation-check"));
assert.ok(readme.includes("Phase21-62"));
assert.ok(readme.includes("PR #256 merged"));
assert.ok(readme.includes("home PC main updated / private-local.html confirmed"));
assert.ok(readme.includes("5b577f6"));
...
const fakeDoc = {
  querySelector(selector) {
    if (selector === "#phase21-62-private-local-post-pr256-continuation-check-list") {
      return {
        textContent: "old",
...
builder.renderPrivateLocalPostPr256ContinuationCheckPanel(panel, fakeDoc);
assert.ok(rendered.length >= 20);
assert.ok(rendered.some((line) => line.includes("PR #256 merged")));
assert.ok(rendered.some((line) => line.includes("5b577f6")));
assert.ok(rendered.some((line) => line.includes("No suspicious auto-run script")));

console.log("Phase21-62 private local post PR256 continuation check builder test passed");
