(() => {
  ...
  const SOURCE_KEYS = ["integrated-os.json", "prediction-engine.json", "learning-engine.json", "profit-db.json", "return-ai-db.json", "history-db.json", "win5-db.json", "trifecta-pattern-db.json", "bankroll-db.json"];
  const DEFAULT_RACES = [
  ...
  const calculateBankrollBudget = ({ currentBankroll = 100000, riskRate = 0.12 } = {}) => Math.max(1000, Math.round(toNumber(currentBankroll, 100000) * riskRate));
  const analyzeWin5ReturnRate = ({ payout = 0, budget = 1 } = {}) => Math.round(toNumber(payout) / Math.max(1, toNumber(budget, 1)) * 100);
  const buildWin5Engine = ({ races = DEFAULT_RACES, bankroll = 100000, riskRate = 0.12 } = {}) => {
    const budget = calculateBankrollBudget({ currentBankroll: bankroll, riskRate });
    const expectedPayout = Math.round(calculateExpectedPayout(races));
    return { version: VERSION, phase: "Phase15", engine: "win5Engine", stableBase: "Official Release v2.7", budget, expectedPayout, expectedValue: expectedPayout - budget, returnRate: analyzeWin5ReturnRate({ payout: expectedPayout, budget }), tickets: generateTickets(races), hitRate: calculateHitRate(races) };
  };
