(() => {
  ...
  const SOURCE_KEYS = ["integrated-os.json", "prediction-engine.json", "learning-engine.json", "profit-db.json", "return-ai-db.json", "win5-db.json", "bankroll-db.json", "history-db.json", "jockey-research-db.json", "trainer-research-db.json", "course-research-db.json", "darkhorse-db.json", "distance-db.json"];
  ...
  const buildEnhancedResearchSummary = ({ storage = window.localStorage } = {}) => ({ sourceCount: SOURCE_KEYS.length, jockeyResearch: readRecords(storage, "jockey-research-db.json", []).length, trainerResearch: readRecords(storage, "trainer-research-db.json", []).length, courseResearch: readRecords(storage, "course-research-db.json", []).length, darkHorseResearch: readRecords(storage, "darkhorse-db.json", []).length, distanceResearch: readRecords(storage, "distance-db.json", []).length, memo: "Version 2.8 Beta research sources expanded without removing v2.7 behavior." });
  const buildResearchLabEngine = ({ storage = window.localStorage } = {}) => {
    const base = buildResearchBase({ storage });
    const summary = buildEnhancedResearchSummary({ storage });
    const modules = {
      jockey: researchJockey(base),
      trainer: researchTrainer(base),
      distance: researchDistance(base),
      course: researchCourseOs(base),
      darkHorse: researchDarkHorse(base)
    };
    return { version: VERSION, phase: "Phase15", engine: "researchLabEngine", stableBase: "Official Release v2.7", modules, summary, integratedRule: generateSelfEvolutionRule(base) };
  };
