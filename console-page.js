...
const RELEASE_STATUS = "Official Release v1.7";
const RELEASE_SCORE = 102;
...
const win5Modules = ["的中率AI", "荒れ度AI", "万馬券確率AI", "資金配分AI", "買い目生成AI", "WIN5回収率AI"];
...
function bootConsolePage() { const key = document.body.dataset.console || "tokyo"; const isWin5 = key === "win5"; const label = isWin5 ? "WIN5 AI" : raceConsoles[key] || "東京版"; const title = isWin5 ? "WIN5 AI Console" : `${label}AI Console`; document.title = `橋本競馬AI Version ${RELEASE_VERSION} / ${title}`; setConsoleText("official-banner-title", `橋本競馬AI ${RELEASE_STATUS}`); setConsoleText("console-title", title); setConsoleText("console-heading", title); setConsoleText("console-subtitle", `${title} / 回収率AI`); setConsoleText("console-version", `橋本競馬AI Version ${RELEASE_VERSION}`); setConsoleText("console-status", RELEASE_STATUS); setConsoleText("console-score", `Release Score ${RELEASE_SCORE}`); if (isWin5) renderTiles("win5-ai-modules", win5Modules); else renderRaceButtons("race-numbers", key); }
