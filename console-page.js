(function () {
  ...
  const consoleDatabase = {
    win5: {
      ...
      cards: [
        ...
        { title: "荒れ度AI", detail: "各対象レースの荒れ度と点数配分を確認します。" },
        { title: "A固定判定AI", detail: "固定候補にできるA馬の信頼度を保護モードで判定します。" },
        { title: "点数圧縮", detail: "買い目点数を圧縮する候補整理ビューです。" },
    ...
    hakodate: {
      ...
      cards: [
        { title: "函館芝1200", detail: "短距離、洋芝、前残りを重点確認します。" },
    ...
    tokyo: {
      ...
      cards: [{ title: "東京コース", detail: "既存の東京版専用導線を維持します。" }],
      links: [createLink("東京 Dashboard", "東京競馬場トップ", "東京競馬場/index.html")]
    }
  };
  ...
  const getConsoleDefinition = (key = "tokyo") => {
    ...
    return {
      ...
      message: `${alias.label}版AI Consoleです。既存専用導線を維持します。`,
  ...
  const normalizeLinkStatusMap = (linkStatusMap = {}) => {
    const normalized = {};
    for (const [href, status] of Object.entries(linkStatusMap || {})) normalized[href] = typeof status === "string" ? { status } : status;
    return normalized;
  };
  ...
  const applyLinkIntegrityFallback = (links = [], linkStatusMap = {}) => {
    ...
    return links.map((link) => {
      ...
      return { ...link, originalHref: link.href, href: link.fallbackHref || SAFE_FALLBACK_HREF, detail: link.fallbackDetail || SAFE_FALLBACK_DETAIL, integrityStatus: "fallback", missingReason: check.reason || "repository_path_missing" };
    });
  };

  const collectConsoleLinks = (keys = ["win5", "hakodate", "sapporo"]) => keys.flatMap((key) => getConsoleDefinition(key).links.map((link) => ({ consoleKey: key, ...link })));
  const setText = (documentRef, id, value) => { const element = documentRef.getElementById(id); if (element) element.textContent = value; };
  const renderCards = (documentRef, targetId, cards = []) => { const target = documentRef.getElementById(targetId); if (target) target.innerHTML = cards.map((card) => `<article class="console-activation-card"><strong>${escapeHtml(card.title)}</strong><span>${escapeHtml(card.detail)}</span></article>`).join(""); };
  const renderLinks = (documentRef, targetId, links = []) => { const target = documentRef.getElementById(targetId); if (target) target.innerHTML = links.map((link) => `<a class="console-link-card" href="${escapeHtml(link.href)}" data-original-href="${escapeHtml(link.originalHref || link.href)}" data-integrity-status="${escapeHtml(link.integrityStatus || "verified")}"><strong>${escapeHtml(link.title)}</strong><span>${escapeHtml(link.detail)}</span></a>`).join(""); };
  const renderPolicyTags = (documentRef) => { const target = documentRef.getElementById("console-policy-tags"); if (target) target.innerHTML = [basePolicy.mode, basePolicy.executionPolicy, basePolicy.protectionPolicy, basePolicy.repositoryPolicy, basePolicy.githubPagesPolicy, "External send disabled", "Auto update disabled", "Link fallback is local only"].map((tag) => `<span class="pill">${escapeHtml(tag)}</span>`).join(""); };
  ...
  const renderConsolePage = (documentRef = document, locationRef = window.location, options = {}) => {
    ...
    const definition = getConsoleDefinition(params.get("console") || "tokyo");
  ...
  if (typeof document !== "undefined") document.addEventListener("DOMContentLoaded", () => renderConsolePage());
  if (typeof module !== "undefined") module.exports = { PHASE, ACTIVATION_PHASE, EXECUTION_POLICY, PROTECTION_POLICY, RELEASE_STATUS, SAFE_FALLBACK_HREF, basePolicy, consoleDatabase, getConsoleDefinition, collectConsoleLinks, createLink, applyLinkIntegrityFallback, renderConsolePage };
})();
