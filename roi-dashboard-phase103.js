(function () {
  ...
  const fallbackDatabase = {
    ...
    records: [
      { date: "2026-06-01", racecourse: "Tokyo", raceName: "Tokyo Value Sprint", betType: "Win", investment: 3000, return: 7200, profit: 4200, roi: 140, hit: true, aiScore: 88, confidence: 84, category: "A" },
      { date: "2026-06-02", racecourse: "Kyoto", raceName: "Kyoto Longshot Cup", betType: "Quinella", investment: 2800, return: 9100, profit: 6300, roi: 225, hit: true, aiScore: 82, confidence: 77, category: "A" },
      { date: "2026-06-03", racecourse: "Chukyo", raceName: "Chukyo Trifecta Logic", betType: "Trifecta", investment: 4800, return: 18800, profit: 14000, roi: 291.7, hit: true, aiScore: 91, confidence: 81, category: "A" },
      { date: "2026-06-06", racecourse: "Kyoto", raceName: "Kyoto WIN5 Finish", betType: "WIN5", investment: 6000, return: 38600, profit: 32600, roi: 543.3, hit: true, aiScore: 89, confidence: 79, category: "A" }
    ]
  };
  ...
  function normalizeRecord(record) {
    ...
    return { ...record, investment, return: payout, profit, roi: investment > 0 ? Number(((profit / investment) * 100).toFixed(1)) : 0, hit: Boolean(record.hit) };
  }
  ...
  function getMonthKey(dateText) { return String(dateText || "").slice(0, 7); }
  function buildPeriodRoi(records, keyFn) {
    return Object.entries(groupBy(records, keyFn)).map(([period, periodRecords]) => ({
      period,
      ...
      hitRate: periodRecords.length ? Number(((periodRecords.filter((record) => record.hit).length / periodRecords.length) * 100).toFixed(1)) : 0
  ...
  function buildDimension(records, values, field) {
    return values.map((value) => {
      ...
      return {
        ...
        count: scoped.length,
        hitRate: scoped.length ? Number(((scoped.filter((record) => record.hit).length / scoped.length) * 100).toFixed(1)) : 0
      };
    });
  }
  function pickBest(items) { return items.filter((item) => item.count > 0).slice().sort((left, right) => right.roi - left.roi || right.profit - left.profit)[0]; }
  function pickWorst(items) { return items.filter((item) => item.count > 0).slice().sort((left, right) => left.roi - right.roi || left.profit - right.profit)[0]; }
  function buildCurves(records, startingBankroll) {
    let bankroll = startingBankroll;
    let peakBankroll = startingBankroll;
    let cumulativeProfit = 0;
    return records.slice().sort((left, right) => `${left.date}-${left.raceName}`.localeCompare(`${right.date}-${right.raceName}`)).map((record) => {
      ...
      return { date: record.date, raceName: record.raceName, profit: record.profit, cumulativeProfit, bankroll, drawdown, roi: record.roi };
    });
  }
  function buildDashboard(database) {
    ...
    return {
      ...
      charts: {
        roiTrend: buildPeriodRoi(records, (record) => record.date).map((point) => ({ label: point.period, value: point.roi })),
        profitTrend: curves.map((point) => ({ label: point.date, value: point.cumulativeProfit })),
        bankrollTrend: curves.map((point) => ({ label: point.date, value: point.bankroll })),
        profitCurve: curves
      }
    };
  }
  ...
  function renderBars(id, points, formatter) {
    ...
    element.innerHTML = points.map((point) => {
      ...
      return `<article><span>${point.label}</span><strong>${formatter(numericValue)}</strong><em style="display:block;width:${width}%;height:8px;background:#d6a84f;border-radius:999px;"></em></article>`;
    }).join("");
  }
  function renderTable(id, rows, nameLabel) {
    const table = document.getElementById(id);
    if (!table) return;
    table.innerHTML = rows.map((row) => `<tr><td>${row[nameLabel]}</td><td>${row.count}</td><td>${yenFormatter.format(row.investment)}</td><td>${yenFormatter.format(row.return)}</td><td>${yenFormatter.format(row.profit)}</td><td>${row.roi}%</td><td>${row.hitRate}%</td></tr>`).join("");
  }
  function renderRecords(records) {
    const table = document.getElementById("phase103-roi-database");
    if (!table) return;
    table.innerHTML = records.map((record) => `<tr><td>${record.date}</td><td>${record.racecourse}</td><td>${record.raceName}</td><td>${record.betType}</td><td>${yenFormatter.format(record.investment)}</td><td>${yenFormatter.format(record.return)}</td><td>${yenFormatter.format(record.profit)}</td><td>${record.roi}%</td><td>${record.hit ? "Hit" : "Miss"}</td><td>${record.aiScore}</td><td>${record.confidence}%</td><td>${record.category}</td></tr>`).join("");
  }
  ...
  async function loadDatabase() {
    ...
    try {
      ...
      if (!response.ok) throw new Error("roiDatabase fetch failed");
  ...
  window.HashimotoPhase103RoiDashboardEngine = { betTypes, buildDashboard, calculateRoi, fallbackDatabase, racecourses, sourceConnections };
