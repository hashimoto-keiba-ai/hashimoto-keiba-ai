(() => {
  ...
  const analyzeProfitEngine = ({ storage = window.localStorage, bankrollRecords = [] } = {}) => {
    const records = readRecords(storage, STORAGE_KEYS.profit, [buildProfitCandidate({ storage })]);
    const totalReturn = records.reduce((sum, item) => sum + toNumber(item.returnRate), 0);
    const averageReturnRate = Math.round(totalReturn / Math.max(1, records.length));
    const roi = Math.round(records.reduce((sum, item) => sum + calculateExpectedReturn(item), 0) / Math.max(1, records.length));
    const bankrollTrend = bankrollRecords.length ? bankrollRecords.map((item) => toNumber(item.currentBankroll, item.initialBankroll)).filter(Boolean) : records.map((item, index) => 100000 + (toNumber(item.returnRate) * (index + 1)));
    return { version: VERSION, phase: "Phase15", engine: "profitEngine", stableBase: "Official Release v2.7", averageReturnRate, roi, bankrollTrend, bestProfitRace: records.slice().sort((a,b)=>toNumber(b.returnRate)-toNumber(a.returnRate))[0]?.race || "--" };
  };
